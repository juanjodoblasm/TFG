...
Veamos la distribución temporal de la cantidad de defunciones de cada causa mediante gráficos de cajas.

```{r distribucion muertes}
total_muertes_año <- data_eu %>%
  select(año, meningitis:hepatitis_aguda) %>%
  group_by(año) %>%
...
total_muertes_agrupadas_año <- data_eu_agrupado %>%
  select(año, enf_infec_paras:enf_cronicas) %>%
  group_by(año) %>%
  summarise(across(everything(), ~ sum(.)))
...
De esta forma, dejaremos únicamente las covariables *poblacion* e *idh* en la base de datos.

```{r eliminacion covariables}
new_data <- data_eu_agrupado %>%
  dplyr::select(-starts_with("emisiones"), -starts_with("esperanza"), -pib)
...
```{r imputacion ceros}
causas_muerte_imp <- causas_muerte %>%
...
Cabe destacar el caso de la variable *total_muertes* que no tiene ninguna correlación considerable exceptuando con *población*, que es una correlación obvia.
...
```{r grafico resumen, fig.width=10, fig.height=7}
par(mfrow = c(2, 2), las = 1)
...
legend("center", legend = nombres_causas, col = colores, lwd = 2, bty = "n")
...
```{r modelo NBMM enf_sist_nervioso, echo=TRUE}
...
```{r modelo NBMM enf_sist_digestivo, echo=TRUE}
...
```{r modelo NBMM_trastornos_embarazo, echo=TRUE}
