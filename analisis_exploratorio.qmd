...
Basándonos en la motivación anterior, proponemos un modelo Binomial Negativo Mixto
...
la heterogeneidad intrínseca entre países, además de un spline cúbico para capturar el efecto del tiempo. La formulación es la siguiente:

$$
\log(\mu_{ijk}) = \log\bigl(N_{ij}\bigr) + \beta_{0k} + \beta_{1k}x_{ij} + \beta_{2k}t_j + \beta_{3k}t_j^2 + \beta_{4k}t_j^3 + u_i,
$$

donde 

-   $\mu_{ijk}$ es la media de muertes por la enfermedad $k$ en el país $i$ en el año $j$.
-   $N_{ij}$ es el total de muertes en el país $i$ en el año $j$.
-   $x_{ij}$ es el IDH del país $i$ en el año $j$.
-   $t_j$ es el año.
-   $\beta_{0k}$, $\beta_{1k}$, $\beta_{2k}$, $\beta_{3k}$, $\beta_{4k}$ son los coeficientes a estimar.
-   $u_i \sim \mathcal{N}(0,\sigma^2)$ es el efecto aleatorio país.

### Estimación del modelo

Para estimar el modelo, primero vamos a reescalar nuestra variable año de forma que caiga en el intervalo $[0,1]$ para evitar posibles errores por *overflow*. Además, al querer un spline cúbico necesitamos las variables año al cuadrado y año al cubo.
...
```{r modelo NBMM enf_sist_nervioso, echo=TRUE, warning = FALSE, message = FALSE}
N = new_data$total_muertes # offset

m_enf_sist_nervioso = glmm.nb(enf_sist_nervioso ~ idh + año2 + año3 + offset(log(N)),
                              random = ~ 1 | pais, data = new_data)

#m_enf_sist_nervioso2 = glmm.nb(enf_sist_nervioso ~ idh + año + año2 + año3 + offset(log(N)),
                              #random = ~ 1 | pais, data = new_data)

summary(m_enf_sist_nervioso)
#anova.lme(m_enf_sist_nervioso2, m_enf_sist_nervioso1)
```
```{r}
residuales = m_enf_sist_nervioso$residuals
sd(residuales[,2])
```



```{r modelo NBMM enf_sist_digestivo, echo=TRUE, warning = FALSE, message = FALSE}
m_enf_sist_digestivo = glmm.nb(enf_sist_digestivo ~ idh + año + año2 + año3 + offset(log(N)),
                              random = ~ 1 | pais, data = new_data)

summary(m_enf_sist_digestivo)
```

```{r modelo NBMM trastornos_embarazo, echo=TRUE, warning = FALSE, message = FALSE}
m_trastornos_embarazo = glmm.nb(trastornos_embarazo ~ idh + año + año2 + año3 + offset(log(N)),
                              random = ~ 1 | pais, data = new_data)
...
```{r}
u = m_enf_sist_nervioso$coefficients$random$pais
sd(u)
```

```{r}
u = m_trastornos_embarazo$coefficients$random$pais
u
plot(density(u))
shapiro.test(u)
```
